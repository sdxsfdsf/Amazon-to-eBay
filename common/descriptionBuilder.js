...
(function (root, factory) {
  ...
  function filterDescriptionAttributes(attributes) {
    return filterAttributes(attributes).filter((a) => !isBlockedDetailLabel(a.label));
  }
  ...
  /** PROMPT ONLY: blank header row, separator, then EVERY real attribute row. */
  function buildPromptAttributesTable(attributes) {
    const rows = filterAttributes(attributes);
    if (!rows.length) return "";
    const labels = rows.map((a) => `**${cleanLabel(a.label)}**`);
    const values = rows.map((a) => String(a.value).trim());
    const lw = Math.max(...labels.map((l) => l.length));
    const vw = Math.max(...values.map((v) => v.length));
    const out = [
      `| ${" ".repeat(lw)} | ${" ".repeat(vw)} |`,
    ...
    for (let i = 0; i < rows.length; i += 1) out.push(`| ${labels[i].padEnd(lw)} | ${values[i].padEnd(vw)} |`);
    return out.join("\n");
  }
   ...
   * Details row is a flex item with `flex:1 1 420px`, so two sit side by
   * side while the container is wide enough for both, wrapping to one per
   * row as it narrows; inside a row, the label and value are themselves
  ...
  const INLINE = {
    ...
    row: "display:flex;flex-wrap:wrap;flex:1 1 420px;min-width:0;max-width:100%;box-sizing:border-box;border-bottom:1px solid #dedbd4;",
    rowLabel:
  ...
  function buildDescriptionHTML(descriptionData) {
    ...
    const extraText = data.standaloneText
      ? escapeHTML(String(data.standaloneText).trim()).replace(/\n+/g, "<br/>")
      : "";
