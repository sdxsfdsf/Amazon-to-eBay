/* global window, ResizeObserver, chrome */
...
(function () {
  ...
  async function createPanel(opts) {
    ...
    const settingsBtn = el("button", { type: "button", class: "aeb-icon", text: "⚙", title: "Settings" });
    ...
    // ---- push page content down by the real rendered height ----
    let offsetHost = document.documentElement;
    const applyOffset = () => {
      const h = root.hidden ? 0 : root.getBoundingClientRect().height;
      offsetHost.style.setProperty("padding-top", `${Math.round(h)}px`, "important");
    };
    new ResizeObserver(applyOffset).observe(root);
    ...
    const api = { root, setStatus, setTempStatus, applyOffset, getSelectedAction: () => selectedAction };
  ...
  function buildSettings(settings, onChanged) {
    ...
    priceCard.append(grid, el("div", { style: "margin-top:8px" }, [saveBtn]));
    ...
    advCard.append(el("div", { style: "margin-top:8px" }, [advSaveBtn]));
