...
console.log("\n[E1] Several attributes -> one full-width row per attribute");
{const h=D.buildDescriptionHTML({attributes:A(9),bullets:["a"]});
 ck("Product Details section present", h.includes('class="aeb-details"'));
 ck("exactly 9 full-width rows rendered", (h.match(/flex:1 1 100%/g)||[]).length===9);
...
{const h=D.buildDescriptionHTML({attributes:A(1),bullets:["a"]});
 ck("exactly one row", (h.match(/flex:1 1 100%/g)||[]).length===1);
...
{const h=D.buildDescriptionHTML({attributes:A(7),bullets:["a"]});
 ck("exactly 7 independent rows", (h.match(/flex:1 1 100%/g)||[]).length===7);
