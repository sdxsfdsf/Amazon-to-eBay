...
function makeHarness({
  ...
  counterMissingWhenEmpty = false,
  renderedImageLimit = Infinity,
} = {}) {
  ...
  const photoSection = {
    isConnected: true,
    get textContent() {
      if (counterMissingWhenEmpty && server.length === 0) return "Photos & Video";
    ...
    querySelectorAll(selector) {
      ...
      if (selector === "img") {
        return server.slice(0, renderedImageLimit).map((name, i) => ({
          isConnected: true,
...
(async () => {
  ...
  console.log("\n[K] Counter appearing after an empty baseline completes verification");
  {
    const h = makeHarness({ fileCount: 7, counterMissingWhenEmpty: true, renderedImageLimit: 6 });
    const res = await h.upload();
    ck("visible 7/25 counter verifies all files even when only 6 img nodes render", res.ok && res.verified === 7, JSON.stringify(res));
    ck("completion detection never resubmits the batch", h.submissions.length === 1, JSON.stringify(h.submissions));
  }
