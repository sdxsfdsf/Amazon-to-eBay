...
(function () {
  ...
  function gallerySnapshot() {
    let root = photoSection() || document;
    let explicit = galleryCounter(root);
    let imageCount = uploadedSrcs(root).size;

    // A heading-only wrapper is not a usable verification scope. The uploader
    // already falls back to the document when eBay renders its real file input
    // outside that wrapper; verification must use the same escape hatch or a
    // successful upload can remain pending until timeout.
    if (root !== document && explicit == null && imageCount === 0 && fileInputsIn(root).length === 0) {
      root = document;
      explicit = galleryCounter(root);
      imageCount = uploadedSrcs(root).size;
    }
  ...
  function galleryDelta(baseline, current) {
    ...
    // Some empty eBay galleries do not render "0/25". If the baseline was
    // demonstrably empty, a counter that appears after submission is the
    // authoritative total added by this operation.
    if (baseline && baseline.counter == null && baseline.count === 0 && current && current.counter != null) {
      deltas.push(Math.max(0, current.counter));
    }
